...
class FE25519 {
  List<int> value = List<int>.filled(10, 0);
}

int GeFromBytesNegateVartime(GE25519P3 h, Uint8List s) {
  final u = FieldElement();
  final v = FieldElement();
  final v3 = FieldElement();
  final vxx = FieldElement();
  final mRootCheck = FieldElement();
  final pRootCheck = FieldElement();
  ...
  FeSquare(h.X, v3);
  FeMul(h.X, h.X, v);
  FeMul(h.X, h.X, u); // x = uv^7

  fePow22523(h.X, h.X); // x = (uv^7)^((q-5)/8)
  FeMul(h.X, h.X, v3);
  FeMul(h.X, h.X, u); // x = uv^3(uv^7)^((q-5)/8)

  FeSquare(vxx, h.X);
  FeMul(vxx, vxx, v);
  FeSub(mRootCheck, vxx, u); // vx^2-u
  if (FeIsNonZero(mRootCheck) != 0) {
    FeAdd(pRootCheck, vxx, u); // vx^2+u
    if (FeIsNonZero(pRootCheck) != 0) {
      return -1;
    }
